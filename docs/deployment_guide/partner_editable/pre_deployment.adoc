...
You need to create an Amazon Simple Storage Service (Amazon S3) bucket in one of the AWS Regions. See AWS documentation on how to https://docs.aws.amazon.com/AmazonS3/latest/userguide/creating-bucket.html[Create your first Amazon Simple Storage Service (Amazon S3) bucket^].

To upload files into your Amazon Simple Storage Service (Amazon S3) bucket, see https://docs.aws.amazon.com/AmazonS3/latest/userguide/uploading-an-object-bucket.html[Upload an object to your bucket^].

This Amazon Simple Storage Service (Amazon S3) bucket is used for storing Red Hat OpenShift pull secret which is required for deploying {partner-product-name}. Additionally, it can also be used to store the optional TLS certificates and keys.
